...
Cameron-Martin theory can initially be confusing because the Cameron-Martin spaces pops up in unexpected ways. Personally, the most intuitive place to start is trying to impose a Hilbert space structure on $\calB$. The idea is that $C_{\mu}$ is almost an inner product on $\calB^*$, it's just positive semi-definite rather than positive definite. We'll first understand the structure $C_{\mu}$ induces on $\calB^*$, then move that structure onto $\calB$.

$\calB^*$ consists of bounded operators $l: \calB \to \mu$. This can be viewed as just a random variable. Since Gaussian measures have finite second measures and $l$ is bounded, we have
\begin{equation}
	\calB^* \subset L^2(\calB, \mu) \defeq \left\{f: \calB \to \R : \int_{\calB} \abs{f(x)}^2 \, \mu(\dif x) < \infty \right\}.
\end{equation}
Then in $L^2$ space, we have
\begin{equation}
	\inn{l}{l'}_{L^2} = \int_{\calB} l(x) l'(x) \, \mu(\dif x) = C_{\mu}(l, l').
\end{equation}
Therefore inner product $C_{\mu}$ induces is exactly the $L^2$ structure on $\calB^*$. Note that \vref{prop:bounded-cov} shows that $\calB^*$ with the usual toplogy induced by $\norm{\cdot}_{\op}$, the operator norm, can be continuously embedded into the $L^2$ structure. This embedding is in fact compact.
\begin{prop}
	$(\calB^*, \norm{\cdot}_{op}) \xhookrightarrow{\text{cpt}} (L^2, \norm{\cdot}_2)$
\end{prop}
\begin{proof}
\end{proof}
We now transfer this to the dual.
