...
Note that the $\lambda_n e_n = \hat{C}_{\mu}(\inn{\cdot}{e_n})$ are in the image of $\hat{C}_{\mu}$. Following this idea:
\begin{defn}
	Let $\mu$ be a Gaussian measure on $\calB$. Then define
	\begin{equation}
		\calH_{\mu} \defeq \left\{ \int_{\calB} x l(x) \mudiff : l \in \calB^* \right\}
		= \hat{C}_{\mu}(\calB^*)
	\end{equation}
	with inner product
	\begin{equation}
		\inn{h}{k}_{\mu} \defeq C_{\mu}(h^*, k^*) = \int_{\calB} h(x) k(x) \mudif
	\end{equation}
	where $h = \hat{C}_{\mu}(h^*)$ and $k = \hat{C}_{\mu}(k^*)$.
\end{defn}
It's not a priori clear that this inner product is well defined.
...
\end{prop}
\begin{proof}
	If (1) holds then there exists $A, B \in \calE$ such that $\mu(A) = 1$, $\nu(B) = 1$ and $A$ and $B$ are disjoint. Then $\nu(A) \leq \nu(B^c) = 0$. Thus $\norm{\mu - \nu}_{TV} \geq \abs{\mu(A) - \nu(A)} = 1$.

	Conversely if $\norm{\mu - \nu}_{TV} = 1$ then there exists $\{A_n\} \subset \calE$ such that
	\begin{equation}
		\lim_n \abs{\mu(A_n) - \nu(A_n)} = 1
	\end{equation}
	The sequence $(\mu(A_n))$ and $(\nu(A_n))$ are bounded so by Bolzano-Weierstrass we can pass to a subsequence along which $\alpha \defeq \lim_n \mu(A_n)$ and $\beta \defeq \lim_n \nu(A_n)$ both exist. Then $\alpha, \beta \in [0, 1]$ thus $\abs{\alpha - \beta} = 1$ means we must have either $\alpha = 0$ or $\beta = 1$.

	WLOG $\alpha = 0$ such that $\lim_n \mu(A_n) \to 0$ and $\lim_n \nu(A_n) = 1$. Then by passing to a subseq WLOG
	\begin{equation}
		\mu(A_n) \leq 2^{-n}.
	\end{equation}
	 Let $A = \{A_n\ \text{i.o.}\}$. Then by the Borel Cantelli lemma, $\mu(A) = 0$. Also
	 \begin{equation}
