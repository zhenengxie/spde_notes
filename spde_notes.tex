...
\end{prop}
\begin{proof}
	By a corollary of the Hahn-Banach theorem, it if $\calA$ is not weak-$*$ dense in $C(\unitbox, \R)^*$ then there exists $L: C(\unitbox, \R)^* \to \R$ linear and continuous w.r.t.\ the weak-$*$ topology such that $L(l) = 0$ for all $l \in \calA$ but $L \not \equiv 0$. But $L$ is continuous w.r.t.\ the weak-$*$ topology if and only if there exists $f \in C(\unitbox, \R)$ such that $L(l) = l(f)$ for all $l \in \calB^*$. But then $f(x) = L(\delta_x) = 0$ for all $x \in \calD$. Thus $f \equiv 0$ by density of $\calD$ and continuity of $f$. Hence $L \equiv 0$, contradiction.
\end{proof}
...
\begin{proof}
	...
	Claim: $M_{\beta}(F) < \infty$ almost surely

	Hence $\hat{F} = \reg_{\beta}(F)$ is a continuous stochastic process $\Omega \to C(\unitbox, \R)$. Let $\mu$ be the law of $\hat{F}$. By construction it is almost surely $\beta$-H\"older continuous thus $\mu(C^{\beta}(\unitbox, \R)) = 1$.
\end{proof}
