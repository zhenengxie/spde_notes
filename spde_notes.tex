...
\begin{defn}[Cameron-Martin Space]
	The following are equivalent definitions of the \emph{Cameron-Martin space} $\calH_{\mu}$.
	\begin{enumerate}
	\item $\calH_{\mu}$ is the completion of
		\begin{equation}
			\mathring{\calH}_{\mu} \defeq \{h \in \calB : \exists h^* \in \calB^*\ \text{with}\ C_{\mu}(h^*, l) = l(h)\ \forall l \in \calB^*\}
		\end{equation}
		which is the image of $\hat{C}_{\mu}$. Set
		\begin{equation}
			\inn{h}{k}_{\mu} - C_{\mu}(h^*, k^*)
		\end{equation}
		where $hat{C}_{\mu}(h^*) = h$ and $\hat{C}_{\mu}(k^*) = k$. By polarisation, it suffices to check this is well defined when $h = k$.
	\end{enumerate}
\end{defn}
...
\begin{prop}[Almost sure convergence implies weak convergence of laws]
	Let $\mu_n, \mu$ be probability measures on a seperable metric space $S$. Suppose $\mu$ has seperable support. Then the following are equivalent:
	\begin{enumerate}
		\item There exists random variables $X_n, X$ defined on a common probability space such that $X_n \sim \mu_n$ for all $n$, $X \sim \mu$, and $X_n \to X$ almost surely.
		\item $\mu_n \towk \mu$.
	\end{enumerate}
\end{prop}
\begin{proof}
	The $(2) \implies (1)$ direction is the hard part (this is the `representation' part) and where seperability of $\supp \mu$ is needed. We'll just need the $(1) \implies (2)$ direction though so that's all we'll prove. Let $f: S \to \R$ be bounded and continuous. Then
	\begin{equation}
