...
\begin{prop}[Fourier transform of Gaussian measures]
	...
	\end{equation}
\end{prop}
\begin{corollary}[Gaussian measures are determined by mean and covariance]
...
\begin{prop}[Boundedness and representation of covariance operators]
...
\begin{prop}[Characterization of Gaussian covariances in Hilbert spaces]
...
The way I like to think about Cameron-Martin theory is perhaps a bit too abstract. Recall $\calB^*$ can be `embedded' into $\calL^2(\calB, \mu)$. Rigirously consider the map $i: \calB^* \to \calL^2$
\begin{equation}
	i(f) = [f]
\end{equation}
which maps $f$ to the equivalence class of $f$ under almost sure equality. We're using $\calL^2$ instead of $L^2$ to distinguish the space of equivalence classes from the space of functions. Then $i$ is a continuous map and the RKHS $\calR_{\mu}$ is $\overline{i(\calB)}$. 

We then want embed $\calR_{\mu}$ into $\calB$. Embedding $\calR_{\mu}$ into $\calB^{**}$ is not too difficult. $i$ has an adjoint map $i^*: (\calL^2)^* \to \calB^{**}$ where $i^*(\phi) = \phi \circ i$ for all $\phi \in (\calL^2)^*$. We don't know if $i^*$ is injective. For that the following proposition from functional analysis helps.
\begin{prop}[Injective of the adjoint map]
	Let $X$ and $Y$ be Banach spaces and $i:X \to Y$ be a bounded linear map. Then the following are equivalent:
	\begin{enumerate}
		\item $i^*: Y^* \to X^*$ is injective.
		\item $i(X)$ is dense in $Y$.
	\end{enumerate}
\end{prop}
This tells us that $\calL^2$ is too big a space to look at, instead we should restrict the range of $i$ from $\calL^2$ to $R_{\mu}$. Then the above proposition tells us that $i^*: \calR_{\mu}^* \to \calB^{**}$ is an injection thus is an embedding of $\calR_{\mu}^* \to \calB^{**}$. Recall by Riesz representation there is a natural bijective linear isometry $\eq : \calR_{\mu} \to \calR_{\mu}^*$ where $\eq(f) \defeq \inn{\cdot}{f}_{L^2}$. Then $i^* \circ \eq$ embeds $\calR_{\mu}$ into $\calB^{**}$. Now let's unravel the action of this embedding, for $l \in \calB^*$.
\begin{align}
	i^* \circ \eq (f)(l) = \inn{i(l)}{f}_{L^2} = \int_{\calB} l(x) f(x) \mudif = l\left(\int_{\calB} x f(x) \mudif \right).
\end{align}
The integral inside the last quantity is a well defined Bochner integral since $\norm{f}_{L^2} < \infty$ guarantees that $x f(x)$ is integrable. This tells us that $i \circ \eq(\calB^*) \subset \ev(\calB)$ thus we can embed $\calR_{\mu}$ into $\calB$ by $k = \ev^{-1} \circ i^* \circ \eq$.

\begin{figure}
	\centering
	\begin{tikzcd}
		\calB^* \ar[r, "i"] &
		\calR_{\mu} \ar[r, <->, "\cong"] \ar[<->]{rrd}[sloped, below]{f \mapsto \int_{\calB} x f(x) \mudif} &
		\calR_{\mu}^* \ar[r, "i^*", <->] &
		\ev(\calH_{\mu}) \\
