...
Cameron-Martin theory is best motiviated by the Hilbert space example. In that example we could express $\mu$ as the law of $\sum_{i=1}^{\infty} \alpha_n \lambda_n e_n$ where $\alpha_n \sim N(0, 1)$ i.i.d.\ and $\lambda_n$ are the eigenvalues of the covariance operator. Note the elements $\{\lambda_n e_n\}$ is an orthonormal basis of $\tilde{C}_{\mu}(\calH)$. The goal is to achieve this for a general Banach space $\calB$.


Unfortunately, orthonormality makes no sense in $\calB$, there is no Euclidean (inner product) structure. So our first goal is to find a subspace of $\calB$ that has a natural Hilbert space structure. In our theory so far what looks most like an inner product? The covariance structure $C_{\mu}: \calB^* \to \calB^*$; it is a symmetric, bilinear, positive semi-definite form on $\calB^*$. Let's explore this structure, 
\begin{equation}
	C_{\mu}(l, l') = \int_{\calB} l(x) l'(x) \mudif = \inn{l}{l'}_{\calL^2}.
\end{equation}
This is just the usual $\calL^2$ structure when we view $\calB^*$ as a subset of
\begin{equation}
	\calL^2 \defeq \{f: \calB \to \R : \int_{\calB} \norm{f(x)}^2 \mudif < \infty \}.
\end{equation}
Formally, we have an embedding $i: \calB^* \to \calL^2$.

Recall $i$ has a corresponding dual embedding $i^*: (\calL^2)^* \to \calB^{**}$
